...
create or replace function check_reto_activo()
	...
	begin
		if TG_OP = 'INSERT' or TG_OP = 'UPDATE' then
			...
			if not(flag)
			then
				raise exception 'No se puede modificar o a?adir a retos inactivos';
			end if;
			return new;
...
create type tipo_actividad as enum
    ('Inauguraci?n', 'Charla Alimentaci?n', 'Charla Motivaci?n', 'Fit Cam');
...
insert into tbl_criterios_calificacion (criterio_calificacion, peso_criterio)
	values
		('Variaci?n de Peso', 0.3),
		('Variaci?n de Medidas', 0.3),
		('Apariencia F?sica', 0.4);
